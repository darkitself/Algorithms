    ...
    public static class HeapSort
    {
        public static void MakeHeapSort(List<string> collection, Func<string, string, int> comparator = default)
        {
            ...
            for (var i = size - 1; i >= 0; i--)
            {
                ...
                Heapify(collection, i, 0, comparator);
            }
        }
            ...
            Func<string, string, int> comparator = default)
        {
            while (true)
            {
                ...
                if (left < size && comparator(collection[left], collection[largest]) > 0) largest = left;

                if (right < size && comparator(collection[right], collection[largest]) > 0) largest = right;
